...
class Music163Spider(scrapy.Spider):
    ...
    def parse(self, response):
        for url in self.start_urls:
            yield SplashRequest(url+('/#/discover/playlist/'), self.parseRequest,endpoint='render.json', args={'wait': 1,'html':1,'iframes': 1,'script':1})
    ...
    def parseRequest(self,response):
        ...
        for se in sel.xpath("//div[@id='cateListBox']/div[@class='bd']//a")[1:]:
            ...
            yield SplashRequest(url,self.parse_gedan,endpoint='render.json', args={'wait': 3,'html':1,'iframes': 1,'script':1})
